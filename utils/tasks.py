"""Helper to locate consumables expiring within the coming week and
send a summary email to admin/TA recipients.

This module intentionally does not depend on Celery. It exposes a
reusable function `notify_consumables_expiring_this_week` that can be
called synchronously (e.g. from a GitHub Actions-triggered webhook)
or from a background worker if you later reintroduce one.
"""
...
def notify_consumables_expiring_this_week() -> bool:
	...
	try:
		...
		items = [c for c in Consumable.query.all() if c.expires and today <= c.expires <= week_end]

	if not items:
		if current_app and current_app.logger:
			current_app.logger.info("No consumables expiring this week; skipping email.")
		return False
	...
	for c in items:
		...
		lines.append(f"- {c.name} (id: {c.id}) — expires: {expires} — qty: {c.quantity} — location: {locname}")
...
# --- Additional helpers: camera gear return reminders and lab equipment service reminders
...
def notify_camera_gear_due_returns(within_days: int = 7) -> bool:
	...
	try:
		items = (
			CameraGear.query.filter(
				CameraGear.is_checked_out == True,
				CameraGear.return_date != None,
		...
		items = [g for g in CameraGear.query.all() if g.is_checked_out and g.return_date and today <= g.return_date <= window_end]

	# include overdue items (return_date < today)
	try:
		overdue = CameraGear.query.filter(CameraGear.is_checked_out == True, CameraGear.return_date < today).all()
	except Exception:
	...
	for g in all_items:
		...
		try:
			...
			checked_out_by = None
		lines.append(f"- {g.name} (id: {g.id}) — return_date: {return_date} — checked_out_by: {checked_out_by} — location: {locname}")
...
def notify_lab_equipment_service_reminders() -> bool:
	...
	for eq, due in due_items:
		...
		lines.append(f"- {eq.name} (id: {eq.id}) — last_serviced_on: {last_serviced} — next_due: {due.isoformat()} — frequency: {eq.service_frequency}")
